...
var enTranslations = map[string]string{
	...
	"清除錯誤":                                         "Dismiss Error",
	"%s\n\n建議排查指引：\n1. 依賴目錄來源：%s\n2. 可能原因：官方目錄尚未發布該版本，或本地設定檔缺少安全雜湊值。\n3. 建議操作：請嘗試在右上角點擊「檢查更新」同步最新設定；若為開發測試環境，請確認 conf/dependencies.json 是否已填入正確的 sha256。": "%s\n\nTroubleshooting Guide:\n1. Dependency Catalog Source: %s\n2. Possible Cause: Official catalog has not published this version yet, or local config lacks security hash.\n3. Recommended Action: Try clicking 'Check Updates' at top right to sync configs. For development/testing environments, verify conf/dependencies.json has the correct sha256.",
