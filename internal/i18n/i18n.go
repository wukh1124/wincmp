...
var enTranslations = map[string]string{
	...
	// 核心依賴缺失提示相關翻譯
	"WinCMP 偵測到您尚未設定所需的 Caddy 核心依賴。": "WinCMP detected that you have not configured the required Caddy dependency.",
	...
	"🛡️ 正在校驗 %s 的完整性...":            "🛡️ Verifying integrity of %s...",
	"❌ 計算 %s 的 SHA-256 失敗":          "❌ Failed to calculate SHA-256 of %s",
	"❌ %s 的完整性校驗失敗":                 "❌ Integrity check failed for %s",
	...
	"正在為 %s 自動配置 Redis 擴充套件 (%s)...":      "Auto-configuring Redis extension (%s) for %s...",
	"自動下載 Redis 擴充失敗: %s":                 "Failed to auto-download Redis extension: %s",
	"自動解壓 Redis 擴充失敗: %s":                 "Failed to auto-extract Redis extension: %s",
	"已成功自動配置 php_redis.dll 至: %s":         "Successfully auto-configured php_redis.dll to: %s",
	"複製 php_redis.dll 至 %s 失敗":            "Failed to copy php_redis.dll to %s",
	"已成功移除 %s 擴充套件":                       "Successfully removed %s extension",
	"已成功移除 PHP %s 執行環境":                   "Successfully removed PHP %s runtime",
	"已成功移除 Redis 快取服務":                    "Successfully removed Redis cache service",
	"已成功移除 Mailpit 服務":                    "Successfully removed Mailpit service",
	"已成功移除 Node.js 環境":                    "Successfully removed Node.js runtime",
	...
	"啟用 Redis 擴充失敗":                       "Failed to enable Redis extension",
	"啟用 php.ini redis 擴充失敗: %w":           "Failed to enable php.ini redis extension: %w",
	"無法連線至 Redis 服務":                      "Unable to connect to Redis service",
	"獲取 Redis DB 資訊失敗":                    "Failed to get Redis DB info",
	"掃描 Redis 鍵值失敗":                       "Failed to scan Redis keys",
	"查詢鍵值型態失敗":                            "Failed to query key type",
	...
	"清除錯誤":                                         "Dismiss Error",
	"%s\n\n建議指引與診斷資訊：\n1. 遠端配置來源 (DependencyURL)：%s\n2. 可能原因：遠端建議設定檔尚未發布該項目版本、SHA-256 遺漏或本地配置缺少雜湊值。\n3. 請嘗試在依賴管理面板點擊「獲取最新」同步設定；若為自訂/測試環境，請確認倉庫分支或本地 dependencies.json 是否已填入正確的 sha256。": "%s\n\nSuggestions and Diagnostic Information:\n1. Remote Config Source (DependencyURL): %s\n2. Possible Cause: Remote recommended config has not published this version yet, SHA-256 is missing, or local config lacks hash.\n3. Try clicking 'Check Updates' in the Dependency Manager to sync configs. For custom/test environments, verify the repository branch or local dependencies.json has the correct sha256.",
	"依賴下載失敗：%v\n\n建議指引與診斷資訊：\n1. 遠端配置來源：%s\n2. 請檢查您的網路連線或代理設定後重試。\n3. 若持續失敗，可手動下載：%s\n4. 並解壓放置於以下 bin 目錄位置：%s":                                              "Failed to download dependency: %v\n\nSuggestions and Diagnostic Information:\n1. Remote Config Source: %s\n2. Check your network connection or proxy settings and retry.\n3. If it persists, you can manually download: %s\n4. And extract it to the following bin directory: %s",
	"SHA-256 完整性校驗失敗！下載的檔案可能損毀、不完整或遭受中間人篡改。\n\n建議指引與診斷資訊：\n1. 遠端配置來源 (DependencyURL)：%s\n2. 請先嘗試在依賴管理面板點擊「獲取最新」，然後重試下載。\n3. 若問題持續，請手動下載：%s\n4. 並解壓放置於以下 bin 目錄位置：%s":   "SHA-256 integrity verification failed! The downloaded file may be corrupted, incomplete, or tampered with.\n\nSuggestions and Diagnostic Information:\n1. Remote Config Source (DependencyURL): %s\n2. Try clicking 'Check Updates' in Dependency Manager first, then retry downloading.\n3. If problem persists, manually download: %s\n4. And extract it to the following bin directory: %s",
	"解壓縮 %s 失敗：%v\n\n建議指引：\n檔案可能損毀。請手動下載：%s\n並解壓放置於以下目錄：%s":                                                                                                           "Failed to extract %s: %v\n\nSuggestions:\nArchive may be corrupted. Please manually download: %s\nand extract to the following directory: %s",
