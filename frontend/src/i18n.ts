...
export const enTranslations: Record<string, string> = {
  ...
  "2. 可能原因：遠端建議設定檔尚未發布該項目版本、SHA-256 遺漏或本地配置缺少雜湊值。": "2. Possible Cause: Remote recommended config has not published this version yet, SHA-256 is missing, or local config lacks hash.",
  "3. 請嘗試在依賴管理面板點擊「獲取最新」同步設定；若為自訂/測試環境，請確認倉庫分支或本地 dependencies.json 是否已填入正確的 sha256。": "3. Try clicking 'Check Updates' in the Dependency Manager to sync configs. For custom/test environments, verify the repository branch or local dependencies.json has the correct sha256.",
