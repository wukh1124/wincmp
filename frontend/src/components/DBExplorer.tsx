  ...
  return (
    <div className="p-6 h-full flex flex-col space-y-6">
      ...
      <div className="flex justify-between items-center select-none">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-white">🗄️ 資料庫瀏覽器 (DB Explorer)</h1>
          <p className="text-xs text-gray-400 mt-1">內建極簡 Schema / 資料表結構速覽，或一鍵透過外部工具管理</p>
        </div>
        <div className="flex gap-2.5">
          ...
          >
            <ExternalLink size={13} /> Open in HeidiSQL
          </button>
        </div>
      </div>
      ...
      {!isRunning ? (
        <div className="flex-1 bg-darkCard border border-darkBorder rounded-xl p-8 flex flex-col items-center justify-center text-center space-y-4 select-none">
          ...
          <div>
            <h3 className="text-sm font-bold text-gray-100">MariaDB 尚未啟動</h3>
            <p className="text-xs text-gray-400 mt-1.5 max-w-sm leading-relaxed">
              請先前往 **Dashboard** 頁面啟動 MariaDB 資料庫服務，再使用 Database Explorer 進行瀏覽。
            </p>
          </div>
        </div>
      ) : (
                ...
                isTablesLoading ? (
                  ...
                  <div className="space-y-3">
                    <div className="text-gray-400 font-bold mb-3 select-none flex items-center gap-1.5 border-b border-darkBorder pb-2 text-[11px]">
                      <Table size={12} className="text-emerald-400" /> 資料庫 '{selectedSchema}' 的資料表：
                    </div>
