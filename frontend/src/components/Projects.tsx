  ...
  return (
    <div className="p-6 overflow-y-auto h-full space-y-6">
      ...
      <div className="flex justify-between items-center select-none">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-white">📁 專案管理面板 (Projects)</h1>
          <p className="text-xs text-gray-400 mt-1">建立並設定專案站點，自適應支援 Laravel、Vite 與各種 Web 專案</p>
        </div>
        <button
          onClick={() => handleOpenEditModal(null, null)}
          className="px-3.5 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-semibold flex items-center gap-1.5 transition duration-200"
        >
          <Plus size={14} /> 新增開發專案
        </button>
      </div>
        ...
        <div className="overflow-x-auto">
          {config?.projects && config.projects.length > 0 ? (
            <table className="w-full text-left text-xs">
              <thead className="bg-[#0f0f12] text-gray-400 uppercase text-[10px] tracking-wider border-b border-darkBorder select-none">
                <tr>
                  <th className="px-6 py-3.5 font-bold">專案名稱 & 根路徑</th>
                  <th className="px-6 py-3.5 font-bold">框架 / Runtime</th>
                  <th className="px-6 py-3.5 font-bold">開發網域 (Domains)</th>
                  <th className="px-6 py-3.5 font-bold">執行狀態</th>
                  <th className="px-6 py-3.5 font-bold">服務啟用</th>
                  <th className="px-6 py-3.5 text-center font-bold">操作</th>
                </tr>
              </thead>
                    ...
                    <tr key={idx} className={`hover:bg-white/[0.015] transition duration-150 ${proj.enabled ? '' : 'opacity-50'}`}>
                      <td className="px-6 py-4">
                        <div className="space-y-0.5">
                          ...
                          <div className="text-[11px] text-gray-500 max-w-[240px] truncate font-mono" title={proj.root_path}>
                            {proj.root_path}
                          </div>
                        </div>
                      </td>
